...
//---------------------------------------------------------
//   collectBurnElementsForLaserMop
...
static void collectBurnElementsForLaserMop(
    Element* parent, const Mop* ll, std::vector<const Element3d*>& out) {
      for (Element* child : parent->children()) {
            ...
            // different LaserMop or to this one via inheritance.
            collectBurnElementsForLaserMop(child, ll, out);
            }
      }
...
std::vector<const Element3d*> LaserMop::collectElements() const {
      ...
      if (!cad)
            return elements;
      collectBurnElementsForLaserMop(cad, this, elements);
      return elements;
      }
...
LaserMop::LaserMop(ZCam* w, Element* parent) : Mop(w, parent) {
      setName("");
      // LaserMop no longer creates its own _geometry.
...
//---------------------------------------------------------
//   processTileLinesLayered
//    Process one tile's geometry through the recipe and return
//    lines grouped by hatch angle.  Outline/wobble lines go into
//    a separate "outline" layer.  No panel-grid offsets.
//---------------------------------------------------------

LayeredLines LaserMop::processTileLinesLayered() const {
      if (!recipe()) {
            Critical("no recipe for <{}>", name());
            return {};
            }

      LayeredLines layered;
      auto elements = collectElements();
      ...
      for (const auto* ce : elements) {
            ...
            auto* ls = &recipe()->pass(0);
            if (ce->pathList().fill()) {
                  createLayeredFill(ll, layered);
                  }
            else if (ls->wobble()) {
                  auto& outlineLayer = PathStrategy::findOrCreateLayer(layered, 0.0, ls, true);
                  for (const auto& p : ll) {
                        if (p.size() < 2)
                              continue;
                        auto wl = wobble(p, ls->wobbleStep(), ls->wobbleSize());
                        outlineLayer.lines.push_back(wl);
                        }
                  }
            else {
                  auto& outlineLayer = PathStrategy::findOrCreateLayer(layered, 0.0, ls, true);
                  for (const auto& l : ll) {
                        ...
                        for (int i = 1; i < l.size(); ++i) {
                              ...
                              currentPoint = l[i];
                              p.push_back(currentPoint);
                              outlineLayer.lines.push_back(p);
                              }
                        }
                  }
            }
...
//---------------------------------------------------------
//   collectLayeredLaserPath
//    Produces layered lines (grouped by hatch angle) for all
//    panel tiles.  Panel-grid offsets are applied.
//    Used by doStartMarking for ebenausweise marking.
//---------------------------------------------------------

LayeredLines LaserMop::collectLayeredLaserPath() const {
      if (!recipe()) {
            Critical("no recipe for <{}>", name());
            return {};
            }
...
//---------------------------------------------------------
//   collectLayeredDisplayLines
//    Return display line segments (mark + move subsets) for all
//    panel tiles, using the layered path strategy.
//    The result has exactly two subsets:
//      subset 0 -- MarkTo segments (if showMarks)
//      subset 1 -- MoveTo segments (if showMoves)
//    No panel-grid offsets are applied.
//---------------------------------------------------------

Clipper2Lib::PathsD LaserMop::collectLayeredDisplayLines() const {
      LayeredLines layered = processTileLinesLayered();

      Clipper2Lib::PathsD lineList;
      Clipper2Lib::PathD markLines;
      Clipper2Lib::PathD moveLines;

      PathStrategy::toDisplayLines(layered, markLines, moveLines, showMarks(), showMoves());

      lineList.push_back(std::move(markLines));
      lineList.push_back(std::move(moveLines));
      return lineList;
      }
...
//---------------------------------------------------------
//   createLayeredFill
//    Fill polygon spdi with hatch pattern, grouping the resulting
//    lines by hatch angle into the given LayeredLines.
//    This is the layer-aware variant of createFill().
//---------------------------------------------------------

void LaserMop::createLayeredFill(Clipper2Lib::PathsD& spdi, LayeredLines& layered) const {
      const LaserPasses* fll = &recipe()->passes();
      ...
      for (int i = 0; i < fll->size(); ++i) {
            ...
            for (int j = 0; j < sl->numPasses(); ++j) {
                  ...
                  // Group lines by normalised angle into HatchLayers.
                  auto& layer1 = PathStrategy::findOrCreateLayer(layered, angle, sl, false);
                  layer1.lines.append_range(lines1);

                  auto& layer2 = PathStrategy::findOrCreateLayer(layered, angle + 90.0, sl, false);
                  layer2.lines.append_range(lines2);
